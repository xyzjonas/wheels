<template>
  <main v-if="selectedVehicle" class="my-3 flex flex-col gap-2">
    ...
    <q-input v-if="includeAmortization" filled v-model="amortization" label="Average Amortizaton" />

    <q-separator class="my-5" />
    <h1 class="text-3xl font-400">
      Grand Total: <strong>{{ total }}</strong> {{ selectedVehicle.setting_currency }}
    </h1>
  </main>
</template>
...
const { avgCostPerDistance, avgCostPerDistanceYear, latestEntry, avgAmortization } =
